before(() => {
  ...
  cy.visit('https://simregpoc.mtnnigeria.net:4334/bow/home/login')
//   cy.visit('https://biowebstaging.mtnnigeria.net/bow/home/login')
  cy.title().should('eq','Biosmartweb')
      })
   ...
   it('Mtn Biosmart Login', function(){
      ...
      cy.get('#newRegMsisdn')
                                          .type('09062058882')
      cy.get('#nrmAdd', {timeout: 10000}).click()
      cy.get('#nrmGoBack > .mat-button-wrapper > .text-white')
                                         .should('contain', 'Back')
      cy.get('body')                     .should('contain', '09062058882' ).should('contain', 'Enter the registration details below')
      // cy.get('body')
      //                                   .should('contain', 'Valid Msisdn.' )
                                       
                                          cy.on('window:alert',(str) =>
                                          {
                                             expect(str).to.equal('Successfully added Msisdn')
                                          }
                                          )
